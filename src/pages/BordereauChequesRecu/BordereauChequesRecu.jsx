...
import { getAllBordereauchequereçus, addBordereauchequereçu, updateBordereauchequereçu, deleteBordereauchequereçu } from '../../api/BordereauCheque/BordereauChequeRecus';
import { CopyPlus, Search, Edit, Trash2, AlertCircle, X, Check } from 'lucide-react';

export default function BordereauChequesRecu() {
  ...
  const fetchBordereaux = async () => {
    try {
      setLoading(true);
      const data = await getAllBordereauchequereçus();
      setBordereaux(data);
      setError('');
    } catch (err) {
  ...
  const handleAddBordereau = async (e) => {
    ...
    setAddLoading(true);
    try {
      const created = await addBordereauchequereçu(newBordereau);
  ...
  const handleUpdateBordereau = async (e) => {
    ...
    setEditLoading(true);
    try {
      const updated = await updateBordereauchequereçu(editingBordereau, editForm);
  ...
  const handleDeleteBordereau = async (id) => {
    try {
      await deleteBordereauchequereçu(id);
  ...
  return (
    <div className='pt-4 px-3 sm:px-8'>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-semibold text-gray-800 dark:text-white">Bordereau de Chèques Reçus</h1>
