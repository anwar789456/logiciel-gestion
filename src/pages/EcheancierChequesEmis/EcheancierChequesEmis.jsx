...
import { getAllEchancierchequeemis, addEchancierchequeemis, updateEchancierchequeemis, deleteEchancierchequeemis } from '../../api/EchancierCheque/EchancierChequeEmis';
import { CopyPlus, Search, Edit, Trash2, AlertCircle, X, Check } from 'lucide-react';

export default function EcheancierChequesEmis() {
  ...
  const fetchEcheanciers = async () => {
    try {
      setLoading(true);
      const data = await getAllEchancierchequeemis();
      setEcheanciers(data);
      setError('');
    } catch (err) {
  ...
  const handleAddEcheancier = async (e) => {
    ...
    setAddLoading(true);
    try {
      const created = await addEchancierchequeemis(newEcheancier);
  ...
  const handleUpdateEcheancier = async (e) => {
    ...
    setEditLoading(true);
    try {
      const updated = await updateEchancierchequeemis(editingEcheancier, editForm);
  ...
  const handleDeleteEcheancier = async (id) => {
    try {
      await deleteEchancierchequeemis(id);
  ...
  return (
    <div className='pt-4 px-3 sm:px-8'>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-semibold text-gray-800 dark:text-white">Échéancier Chèques Émis</h1>
