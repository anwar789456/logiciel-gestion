...
import { getAllBordereautraiterecus, addBordereautraiterecus, updateBordereautraiterecus, deleteBordereautraiterecus } from '../../api/BordereauTraites/BordereauTraitesRecus';
import { CopyPlus, Search, Edit, Trash2, AlertCircle, X, Check } from 'lucide-react';

export default function BordereauTraitesRecu() {
  ...
  const fetchBordereaux = async () => {
    try {
      setLoading(true);
      const data = await getAllBordereautraiterecus();
      setBordereaux(data);
      setError('');
    } catch (err) {
  ...
  const handleAddBordereau = async (e) => {
    ...
    setAddLoading(true);
    try {
      const created = await addBordereautraiterecus(newBordereau);
  ...
  const handleUpdateBordereau = async (e) => {
    ...
    setEditLoading(true);
    try {
      const updated = await updateBordereautraiterecus(editingBordereau, editForm);
  ...
  const handleDeleteBordereau = async (id) => {
    try {
      await deleteBordereautraiterecus(id);
  ...
  return (
    <div className='pt-4 px-3 sm:px-8'>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4 sm:mb-6">
        <h1 className="text-lg sm:text-2xl font-semibold text-gray-800 dark:text-white">Bordereau de Traites Reçus</h1>
              ...
              <form onSubmit={handleAddBordereau} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Émetteur <span className="text-red-500">*</span></label>
                    <input type="text" value={newBordereau.emetteur} onChange={(e) => setNewBordereau({...newBordereau, emetteur: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" required /></div>
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">N° Chèque <span className="text-red-500">*</span></label>
                    <input type="text" value={newBordereau.num_cheque} onChange={(e) => setNewBordereau({...newBordereau, num_cheque: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" required /></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Téléphone</label>
                    <input type="text" value={newBordereau.tel} onChange={(e) => setNewBordereau({...newBordereau, tel: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Banque</label>
                    <input type="text" value={newBordereau.banque} onChange={(e) => setNewBordereau({...newBordereau, banque: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Montant</label>
                    <input type="text" value={newBordereau.montant} onChange={(e) => setNewBordereau({...newBordereau, montant: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Ordre de</label>
                    <input type="text" value={newBordereau.ordre_de} onChange={(e) => setNewBordereau({...newBordereau, ordre_de: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Date Émission</label>
                    <input type="date" value={newBordereau.date_emission} onChange={(e) => setNewBordereau({...newBordereau, date_emission: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
                  <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Échéance</label>
                    <input type="date" value={newBordereau.echeance} onChange={(e) => setNewBordereau({...newBordereau, echeance: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
                </div>
                <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Lieu</label>
                  <input type="text" value={newBordereau.lieu} onChange={(e) => setNewBordereau({...newBordereau, lieu: e.target.value})} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white" /></div>
