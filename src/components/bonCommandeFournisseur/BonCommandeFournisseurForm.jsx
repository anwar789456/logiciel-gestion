...
import { createBonCommandeFournisseur, updateBonCommandeFournisseur } from '../../api/BonCommandeFournisseur/BonCommandeFournisseur';
import toast from 'react-hot-toast';

export default function BonCommandeFournisseurForm({ existingBon, onSuccess, onCancel }) {
  ...
  const [formData, setFormData] = useState({
    ...
    articles: [{ description: '', ref_couleur: '', quantity: '', pu_ht: '', pht: '', pu_ttc: '' }],
    avance: '',
    reglement: ''
  });
  ...
  useEffect(() => {
    if (existingBon) {
      setFormData({
        ...
        date_bon: existingBon.date_bon ? new Date(existingBon.date_bon).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        articles: existingBon.articles?.length ? existingBon.articles : [{ description: '', ref_couleur: '', quantity: '', pu_ht: '', pht: '', pu_ttc: '' }],
  ...
  const addArticle = () => {
    setFormData(prev => ({
      ...prev,
      articles: [...prev.articles, { description: '', ref_couleur: '', quantity: '', pu_ht: '', pht: '', pu_ttc: '' }]
    }));
  };
  ...
  const calculateTotalHT = () => {
    return formData.articles.reduce((sum, article) => {
      return sum + (parseFloat(article.pht) || 0);
    }, 0).toFixed(2);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (existingBon) {
        await updateBonCommandeFournisseur(existingBon._id, formData);
        toast.success('Bon de commande modifié avec succès');
      } else {
        await createBonCommandeFournisseur(formData);
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-6xl h-[95vh] overflow-hidden shadow-xl flex flex-col">
        ...
        <div className="flex-shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-3 sm:px-6 py-3 sm:py-4 flex justify-between items-center">
          <h2 className="text-base sm:text-xl font-semibold text-gray-900 dark:text-white">
            {existingBon ? 'Modifier' : 'Nouveau'} Bon de Commande Fournisseur
          </h2>
          <button
            onClick={onCancel}
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-6">
          <form id="bon-commande-form" onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <div key={index} className="grid grid-cols-12 gap-3 items-start p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <div className="col-span-6 md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={article.pht}
                    onChange={(e) => handleArticleChange(index, 'pht', e.target.value)}
                    autoComplete="off"
                    className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                  />
                </div>
                ...
                <div className="col-span-6 md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={article.pu_ttc}
                    onChange={(e) => handleArticleChange(index, 'pu_ttc', e.target.value)}
                    autoComplete="off"
                    className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                  />
                </div>
