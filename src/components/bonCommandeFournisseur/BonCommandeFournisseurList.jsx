...
import { getAllBonCommandeFournisseurs, deleteBonCommandeFournisseur } from '../../api/BonCommandeFournisseur/BonCommandeFournisseur';
import { exportBonCommandeFournisseurToExcel } from '../../utils/excelExport';
import toast from 'react-hot-toast';

export default function BonCommandeFournisseurList({ onCreateNew, onEdit, onView }) {
  ...
  const fetchBons = async () => {
    try {
      setLoading(true);
      const data = await getAllBonCommandeFournisseurs();
      setBons(data);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await deleteBonCommandeFournisseur(id);
  ...
  const filteredBons = bons.filter(bon => {
    ...
    const matchesSearch = bon.fournisseur?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bon.compteur?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bon.gsm?.includes(searchTerm);
    
    // Date range filter
    const bonDate = bon.date_bon ? new Date(bon.date_bon) : null;
  ...
  const calculateTotal = (articles) => {
    if (!articles || articles.length === 0) return '0.00';
    return articles.reduce((sum, article) => {
      const pht = parseFloat(article.pht) || 0;
      return sum + pht;
    }, 0).toFixed(2);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Rechercher par fournisseur, compteur ou GSM..."
            value={searchTerm}
