...
const TableDisplay = React.forwardRef(({ 
  ...
  const formatValue = (value, type = 'text') => {
    ...
    switch (type) {
      ...
      case 'status':
        const statusStyles = {
          'draft': 'bg-gradient-to-r from-gray-50 to-gray-100 text-gray-700 border-gray-200',
          'sent': 'bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 border-blue-200',
          'accepted': 'bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 border-green-200',
          ...
          'partial': 'bg-gradient-to-r from-orange-50 to-orange-100 text-orange-700 border-orange-200',
          'delivered': 'bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 border-green-200'
        };
        return (
          <span className={`px-3 py-1.5 rounded-full text-xs font-semibold border ${statusStyles[value] || 'bg-gray-50 text-gray-700 border-gray-200'}`}>
            {value}
          </span>
        );
      default:
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {actions.map((action, actionIndex) => (
                              <button
                                ...
                                className={`p-2 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md transform hover:scale-110 ${action.className || 'bg-gray-100 hover:bg-gray-200 text-gray-700 hover:text-gray-900'}`}
                                title={action.title}
                              >
                                {action.icon}
                              </button>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="relative" ref={itemsPerPageDropdownRef}>
                ...
                >
                  <span>Afficher: {itemsPerPage}</span>
                  <ChevronDown className="h-4 w-4" />
                </button>
                
                {showItemsPerPageDropdown && (
                  <div className="absolute mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto">
                    <ul className="py-1">
                      {[10, 20, 50].map((value) => (
                        <li key={value}>
                          <button
      ...
      {loading && data.length === 0 && (
        <div className="absolute inset-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm flex items-center justify-center z-10">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">Chargement...</p>
          </div>
        </div>
      )}
    </div>
